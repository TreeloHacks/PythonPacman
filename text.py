...
class TextGroup(object):
    ...
    def setupText(self):
        size = TILEHEIGHT
        self.alltext[SCORETXT] = Text("o" .zfill(8), WHITE, 0, TILEHEIGHT, size)
        self.alltext[LVLTXT] = Text(str(1).zfill(3), WHITE, 23*TILEWIDTH, TILEHEIGHT, size)
        self.alltext[RDYTXT] = Text("READY!", YELLOW, 11.25*TILEWIDTH, 20*TILEHEIGHT, size, visible=False)
        self.alltext[PAUSETXT] = Text("PAUSED", YELLOW, 10.625*TILEWIDTH, 20*TILEHEIGHT, size, visible=False)
        self.alltext[GMOVRTXT] = Text("GAME OVER!", YELLOW, 10*TILEWIDTH, 20*TILEHEIGHT, size, visible=False)
    ...
    def hideText(self):
        self.alltext[RDYTXT].visible = False
        self.alltext[PAUSETXT].visible = False
        self.alltext[GMOVRTXT].visible = False

    def updateScore(self, score):
        self.updateText(SCORETXT, str(score).zfill(8))

    def udpateLevel(self, level):
        self.updateText(LVLTXT, str(level + 1).zfill(3))
